...
function showUpdateUserInfo(event) {
	...
	$('#updateUserId').val(thisUserObject._id);
};
...
function addUser(event) {
	...
	if(errorCount === 0) {
		...
		alert('Add: Please fill in all fields');
		return false;
	}
};
...
function updateUser(event) {
	...
	if(errorCount === 0) {
		...
		// Use AJAX to put the object to our adduser service
		$.ajax({
			type: 'PUT',
			data: updateUser,
